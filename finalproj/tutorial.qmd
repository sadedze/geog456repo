...
I then saved this new dataset as a JSON. See part of the dataset below.
...
Next, in visual studio code, I created an html document and made a simple leaflet map centered on North Carolina. I first copied the CSS stylesheet, the source script. The style sheet and the source script allow you to use commands and styles specific to leaflet.  
...
Then I created a `div` (a container) for the map and gave it an `id`. The `id` allows me to then give the map dimensions in the CSS code by `#map`. To reference an object `id` in CSS, use a #. 
...
Next, I created a leaflet map centered on North Carolina and added an OSM tile layer to the map. Source: Leaflet Quick Start Guide
...
Here is what the first iteration of my map looked like with each road closure point appearing as an “x” on the map.
...
First I make the dates compatible with the slider range (0 to 17). I define variables for the start and end date of my date range so I can manipulate them more easily. I need to make September 12th = 0 and September 29th = 17. getTime() is a JavaScript command that works with Date objects and returns the numeric value corresponding to the specified time. I calculate each date as a proportion of 17 so it corresponds to the value of the slider.
...
To create a heatmap, I used a leaflet heatmap plugin called Leaflet.heat. First, add the style sheet and the source script to the html. 
...
### Photo of Heat Map and Point Layer w/o 



## Customization and Aesthetics

This is a functional map. But I want to add some customizable features for the user, and add some information to let the reader know what I’m visualizing. 
...
The map is looking better, but the heat layer and point layer on top of one another make the visualization muddled. 

Next, I created added layer switching controls for both layers. I defined the two layers, added the baselayer control object to the map, and added both layers to the map.
...
<body>
    ...
    <div id="map-container3">
        <div id="sidebar3">
            <h1>NC Road Closures During Hurricane Florence</h1>
            <h2>Date: <span id="dateDisplay3"></span></h2>
...
function updateDateDisplay(date) {
        //create var that updates date display element at top of page
        var dateDisplay3 = document.getElementById('dateDisplay3');
        //Transform date into readable text
        dateDisplay3.textContent = date.toDateString();
        }
    //Add layer control to toggle back and forth between layers
        var baseLayers = {
            "Heatmap": heatLayer,
...
quarto: https://quarto.org/docs/output-formats/html-code.html qmd vs code? : https://quarto.org/docs/tools/vscode.html
